...
import { measureScreenCornerRadius } from './frame-measure.js';
...
for (const skin of skins) {
  ...
  const backPath = path.join(outDir, 'back.webp');
  copyFileSync(path.join(skinDir, 'back.webp'), backPath);
  copyFileSync(path.join(skinDir, 'mask.webp'), path.join(outDir, 'mask.webp'));
  // Prefer the corner radius measured from the artwork over the layout's (often absent) value.
  const measured = await measureScreenCornerRadius(backPath, {
    x: geo.offset.x,
    y: geo.offset.y,
    width: geo.display.width,
    height: geo.display.height,
  });
  const manifest = buildFrameManifest(skin, geo, measured ?? undefined);
